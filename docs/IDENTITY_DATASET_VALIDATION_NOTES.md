...
| CI | `^\d{2}[A-Z]{2}\d{5}$` | passport | passport | local sample_verified | Local Cote d'Ivoire operators confirmed the real display format from field samples, including `13AS85673`. |
| CI | `^CI\d{9}$` | national_id | national_id | local sample_verified | Local Cote d'Ivoire operators confirmed the real display format from field samples, including `CI000910000`. |
| CI | `^[A-Z]{4}\d{2}-\d{2}-\d{8}[A-Z]$` | driver_license_number | driver_license_number | local sample_verified | Local Cote d'Ivoire operators confirmed the real driver licence display format from field samples: `LLLLNN-NN-NNNNNNNNL`. |
