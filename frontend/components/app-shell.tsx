              ...
              <div key={item.id} className="rounded-xl border border-slate-800 bg-slate-950 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{item.username} • {item.documentQuality} • {new Date(item.createdAt).toLocaleString()}</p>
                  </div>
          ...
          {!detail ? <p className="text-sm text-slate-400">Generate or open a history row.</p> : (
            <div className="space-y-4 text-sm">
              <div className="grid gap-3 rounded-xl bg-slate-950 p-4">
                ...
                <div>
                  <strong>Password:</strong> {showPassword ? detail.emailPassword : '••••••••••••'}{' '}
                  <button className="rounded border border-slate-700 px-2 py-1 text-xs" onClick={() => setShowPassword((v) => !v)}>{showPassword ? 'Hide' : 'Reveal'}</button>
                </div>
                <p><strong>Username:</strong> {detail.username}</p>
                <p><strong>Role:</strong> {detail.role}</p>
                <p><strong>Document:</strong> {detail.documentType} = {detail.documentValue}</p>
                <p><strong>Quality:</strong> {detail.documentQuality}</p>
                <p><strong>Registration:</strong> <a href={detail.registrationUrl} target="_blank">{detail.registrationUrl}</a></p>
              </div>
              <div className="rounded-xl bg-slate-950 p-4">
                <p className="mb-2 font-medium">Inbox plain text</p>
                <pre className="whitespace-pre-wrap text-slate-300">{detail.inbox.plainText}</pre>
              </div>
              <div className="grid gap-4 md:grid-cols-2">
                <div className="rounded-xl bg-slate-950 p-4">
                  <p className="mb-2 font-medium">Extracted links</p>
                  <ul className="space-y-2">{detail.inbox.links.map((link) => <li key={link}><a href={link} target="_blank">{link}</a></li>)}</ul>
                </div>
                <div className="rounded-xl bg-slate-950 p-4">
                  <p className="mb-2 font-medium">Extracted codes</p>
                  <ul className="space-y-2">{detail.inbox.codes.map((code) => <li key={code}>{code}</li>)}</ul>
                </div>
              </div>
